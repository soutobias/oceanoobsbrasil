...
class SpotterData:
    ...
    def get(
        ...
        for spotter in self.spotters:
            if spotter.id in self.ids:
                ...
                wind = pd.json_normalize(
                    spt_data, record_path=["wind"], meta=["spotterId"]
                )
                wave = pd.json_normalize(
                    spt_data, record_path=["waves"], meta=["spotterId"]
                )
                if "surfaceTemp" in spt_data.keys():
                    sst = pd.json_normalize(
                        spt_data, record_path=["surfaceTemp"], meta=["spotterId"]
                    )
                else:
                    sst = None
        ...
        if save_bd:
            ...
            self.db.feed_bd(
                table="data_no_stations", df=self.result, data_type="drifter"
            )
        else:
            return self.result
    ...
    def merge_values(self, wind, wave, sst, spotter):
        ...
        if not wind.empty:
            ...
            wind_spotter.loc[:, "timestamp"] = pd.to_datetime(
                wind.loc[:, "timestamp"], format=format_date
            )
        else:
            print(
                "No wind data for buoy %s from %s to %s"
                % (spotter.id, self.start_date, self.end_date)
            )
            wind_spotter = pd.DataFrame(columns=wind_columns)

        if not wave.empty:
            waves_spotter = wave[wave_columns]
            waves_spotter.loc[:, "timestamp"] = pd.to_datetime(
                waves_spotter.loc[:, "timestamp"], format=format_date
            )
        else:
            print(
                "No wave data for buoy %s from %s to %s"
                % (spotter.id, self.start_date, self.end_date)
            )
        ...
        if not sst.empty:
            ...
            sst_spotter.loc[:, "timestamp"] = pd.to_datetime(
                sst_spotter.loc[:, "timestamp"], format=format_date
            )
        else:
            print(
                "No sst data for buoy %s from %s to %s"
                % (spotter.id, self.start_date, self.end_date)
            )
            sst_spotter = pd.DataFrame(columns=sst_columns)

        spotter_general = wind_spotter.merge(
            waves_spotter, on="timestamp", how="left"
        ).merge(sst_spotter, on="timestamp", how="left")

        spotter_general.rename(
            columns={
                "speed": "wspd",
                "direction": "wdir",
