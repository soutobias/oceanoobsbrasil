...
class GetData:
    ...
    def get(self, table, start_date=None, end_date=None, last=None, **kwargs):
        if table != "stations":
            if last:
                ...
                if end_date == None:
                    end_date = (datetime.utcnow() + timedelta(days=1)).strftime(
                        "%Y-%m-%d"
                    )
    ...
    def engine_create():
        ...
        engine = create_engine(
            f"postgresql+psycopg2://{os.getenv('POSTGRE_USER')}:{password}@{os.getenv('POSTGRE_LOCAL')}/{os.getenv('POSTGRE_BD')}"
        )
