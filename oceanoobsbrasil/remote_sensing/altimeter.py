...
class Altimeter:
    ...
    def get(self):
        ...
        for f in self.nc_files:
            ...
            flag = (
                NC.groups["data_01"]
                .groups["ku"]
                .variables["swh_ocean_compression_qual"]
            )
            ...
            if not self.result.empty:
                ...
                self.result = (
                    self.result.set_index("date_time")
                    .resample("15S")
                    .first()
                    .reset_index()
                )
                ...
                self.db.feed_bd(
                    table="data_no_stations", df=self.result, data_type="altimeter"
                )
                print("ok")
    ...
    def get_nc_files(self):
        ...
        self.nc_files = list(
            pd.DataFrame(np.array([index, self.nc_files]).T)
            .set_index(0)
            .sort_index()[1]
        )
    ...
    def download_ftplib_nodc(self):
        ...
        for file_name in file_names_filter:
            ...
            urllib.request.urlretrieve(
                self.url + self.directory + url2 + file_name, f"{self.path}/{file_name}"
            )
        ...
        if self.start_date.day != self.end_date.day:
            ...
            for file_name in file_names_filter:
                print(file_name)
                urllib.request.urlretrieve(
                    self.url + self.directory + url2 + file_name,
