    ...
    def get(self):
        ...
        for collection in self.datasets:
            ...
            for f in self.nc_files:
                ...
                wind_flag = pd.concat(
                    [wind_flag, pd.DataFrame(ds["wvc_quality_flag"].values)], axis=0
                )
                wind_dir = pd.concat(
                    [wind_dir, pd.DataFrame(ds["wind_dir"].values)], axis=0
                )
                wind_speed = pd.concat(
                    [wind_speed, pd.DataFrame(ds["wind_speed"].values)], axis=0
                )
                wind_time = pd.concat(
                    [wind_time, pd.DataFrame(ds["time"].values)], axis=0
                )
        ...
        if not self.result.empty:
            ...
            self.db.feed_bd(
                table="data_no_stations", df=self.result, data_type="scatterometer"
            )
        else:
            print("Não há dados")
