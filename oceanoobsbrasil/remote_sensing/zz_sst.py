...
class Mur:
    def __init__(
        ...
        start_date=datetime.strftime(
            datetime.utcnow() - timedelta(days=3), format="%Y-%m-%dT%H:%M:%SZ"
        ),
        ...
        if self.mur_last_date.empty:
            ...
            self.start_date = datetime.strftime(
                mur_last_date["date_time"].iloc[0] + timedelta(days=1),
    ...
    def get(self):
        ...
        for index, point in self.points.iterrows():
            ...
            r = requests.get(
                f"https://coastwatch.pfeg.noaa.gov/erddap/griddap/jplMURSST41.json?analysed_sst%5B({self.start_date}):1:({self.end_date})%5D%5B({lat}):1:({lat})%5D%5B({lon}):1:({lon})%5D"
            )
            json_file = r.json()
