...
class GlossTide():
    ...
    def __init__(self, equip='tide',
        ...
        self.stations = self.db.get(table='stations', institution=['=', 'gloss'], data_type=['=', self.equip])

    def get(self):
        for index, station in self.stations.iterrows():
            query = {'startTime': self.start_date,
            ...
            self.result = pd.concat([self.data,  self.data2], axis=1, join='inner').reset_index()
